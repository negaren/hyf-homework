...
FROM (user_task JOIN task ON task.id = user_task.task_id ) JOIN user ON user.id = user_task.user_id
WHERE user.email LIKE '%@spotify.com';

SELECT user.name as user_name, task.title, status.name as status_name
FROM user_task JOIN user ON user.id = user_task.user_id 
