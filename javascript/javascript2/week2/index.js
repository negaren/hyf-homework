...
console.log(oddNumbers.map(elem => elem * 2));
...
const shortTitleMovies = movies.filter(movie => { //Create an array of movies containing the movies with a short title (you define what short means)
    return movie.title.length < 5;
});
console.log(shortTitleMovies) 

const longTitleMovies = movies.filter(movie => { //Create an array of movie titles with long movie titles
    return movie.title.length > 5;
});
console.log(longTitleMovies)

//Count the number of movies made between 1980-1989 (including both the years)
console.log(movies.filter(movie => (movie.year < 1990 && movie.year > 1979)).length)
...
//Create a new array that has an extra key called tag. The tag is based on the rating: Good (>= 7), Average (>= 4 and < 7), Bad (< 4)
    ...
    movies.forEach(movie => {
        ...
        else Object.assign(movie, {"tag":"Bad"})
    });
        console.log(movies);
...
const moviesHigherT6 = movies.filter(movie => movie.rating > 6 );
const resultMoviesHigherT6 = moviesHigherT6.map(movie => movie.rating);
console.log(resultMoviesHigherT6);
...
const keyWordMovies = movies.filter(movie => {
    const caseInsenMovie = movie.title.toLowerCase();
    const keyarr = ["Alien", " Benjamin", "Surfer"]
    return caseInsenMovie.includes(`${keyarr[0].toLowerCase()}`) || caseInsenMovie.includes(`${keyarr[1].toLowerCase()}`) || caseInsenMovie.includes(`${keyarr[2].toLowerCase()}`) ;
...
let duplicatedIndexArr = [];
movies.forEach(movie => {
   Object.assign(movie, {"sliptedTitle": movie.title.replace(/[^\w\s\d]/gi,'').replace('_','').split(" ")});

})
console.log(movies)
movies.forEach(movie => {
    for (let i = 0; i < movie.sliptedTitle.length; i++){
        for(let j = 0; j < movie.sliptedTitle.length; j++){
            if(movie.sliptedTitle[i] == movie.sliptedTitle[j] && (i !== j)){
                duplicatedIndexArr.push({"title":movie.title});
                return;
             }
        }
...
console.log(duplicatedIndexArr)

// Calculate the average rating of all the movies using reduce. Optional

const ratingArr = movies.filter(movie => movie.rating != undefined).map(movie => movie.rating);

function getAverage(arr){
    return console.log(arr.reduce((total , current) => total + current) / ratingArr.length);
}
getAverage(ratingArr)
...
movies.forEach(movie => {
    ...
    if (movie.tag == "Average"){
        averageMovies.push(1);
    }
    badMoovies.push(1);
})

function movieQuality(arr1, arr2, arr3){
    const total1 = arr1.reduce((total, current) => total + current)
    const total2 = arr2.reduce((total, current) => total + current)
    const total3 = arr3.reduce((total, current) => total + current)
    return console.log(`Good Movies: ${total1}, Average Movies: ${total2}, Bad Movies: ${total3}`)
}

movieQuality(goodMovies, averageMovies, badMoovies);
